	...
	*/
	...
	public boolean equals(Object o) {
		...
		for (int i = 0; i < this.length(); i++) {
			if (!myWords[i].equals(other.getWord(i))) {
				status = false;
			}
		}
	...
	public WordGram shiftAdd(String last) {
		...
		String[] newl = new String[this.length()];

		for (int i = 0; i < newl.length-1; i++) {
			newl[i] = this.getWord(i+1);
		}
		newl[newl.length-1] = last;

		WordGram wordgram = new WordGram(newl, 0, newl.length);
		return wordgram;
	}
	...
	public String toString() {
		...
		String fin = String.join(" ", myWords);
		fin = fin.trim();
